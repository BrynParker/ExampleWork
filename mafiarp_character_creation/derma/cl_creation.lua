...
function PANEL:Init()
	...
	self.content:DockMargin(sideMargin, 30, sideMargin, 0)
	self.content:DockPadding(0,0,0,ScreenScale(23))
	self.content:SetDrawBackground(false)
	self.content.Paint = function(self, w, h)
		surface.SetDrawColor(255, 255, 255, 255)
		surface.SetMaterial(Material("mafiarp/ground_shadow.png", "noclamp smooth"))
		surface.DrawTexturedRect(ScrW() * 0.245, ScrH() * 0.375, 500, 500)
	end

	self.model = self.content:Add("ixModelPanel")
	self.model:SetWide(ScrW() * 0.25)
	self.model:Dock(LEFT)
	self.model:DockMargin(ScreenScale(160),0,0,0)
...
function PANEL:UpdateModel()
	...
	local armCross = self.model:GetEntity():LookupSequence("pose_standing_02")
	self.model:GetEntity():SetSequence(armCross)

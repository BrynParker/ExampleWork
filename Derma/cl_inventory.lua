...
local mainbg = Material ("sci_char/bg_1.png")
local invblock = Material ("sci_char/menu/character_frame.png")
local droplocation = Material ("sci_char/current_stage.png")
			...
			for x = 0, item.gridW - 1 do
				for y = 0, item.gridH - 1 do
					...
					if (self.slots[x2] and IsValid(self.slots[x2][y2])) then
						...
						if (bool) then
							surface.SetDrawColor(255, 29, 255, 20)
						else
						...
						surface.SetMaterial(droplocation)
						surface.DrawTexturedRect((x2 - 1)*64 + 4, (y2 - 1)*64 + 27, 64, 64)
					else

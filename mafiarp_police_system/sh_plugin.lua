local PLUGIN = PLUGIN
PLUGIN.name = "Police CAD/Interaction"
PLUGIN.desc = ""
...
PLUGIN.PoliceRankGroup = "MDPD" -- the name of the group that was set in the advanced rank system
...
PLUGIN.CarsWithLaptop = {
	["Airboat"] = true,
	["Chair_Office2"] = true,
	["lvs_wheeldrive_monaco_cop"] = true,
}
...
PLUGIN.PoliceCarVendor = {
    {
        CarEnt = "lvs_wheeldrive_monaco_cop",
        Icon = "materials/mafiarp_police/police_car.png",  -- Path to the PNG image
        carModel = "models/diggercars/dodge_monaco/monaco_police.mdl",  -- Path to the PNG image
        IsTaken = false
    },
...
if (SERVER) then
	...
	net.Receive("ixPoliceSys_TakePoliceCar", function(len, client)
		...
		if not client:isPolice() then return end
		...
		if not car_data then
			client:Notify("Car data doesn't exist")
			return end
		...
		for platform, _ in pairs(ix_policespawner_platforms) do 
			if platform:GetClass() == "j_police_spawner_platform" then
	
				-- If you want to skip collision check, remove this condition
				if not platform:CheckCollision() then continue end
	
				local CarClass = car_data.CarEnt
				local CarModel = car_data.carModel
	
				-- Spawn the entity directly based on CarClass
				local CarE = ents.Create(CarClass)
				if not IsValid(CarE) then
					ErrorNoHalt("Failed to create entity with class: " .. CarClass .. "\n")
					continue
				end
	
				-- Set the position and angles based on the platform
				CarE:SetPos(platform:GetPos())
				CarE:SetAngles(platform:GetAngles())
	
				-- Set any additional properties like skin, bodygroups, etc.
				CarE:SetModel(CarModel)  -- This assumes the entity already has a model assigned
				--CarE:SetSkin(car_data.Skin or 0)
				CarE.TakenSlot = selected_car
				CarE.PoliceOfficer = client
	
				-- Set bodygroups if any are defined in car_data
				for bgName, bgValue in pairs(car_data.BodyGroups or {}) do
					local bodyID = CarE:FindBodygroupByName(bgName)
					if bodyID and bodyID >= 0 then
						CarE:SetBodygroup(bodyID, bgValue)
					end
				end
	
				-- Setup cleanup when the vehicle is removed
				...
				-- Set fuel if specified
				if car_data.Fuel then
					timer.Simple(1, function()
						if IsValid(CarE) then
				...
				return  -- Exit after successfully spawning the vehicle
			end
		end
	...
	timer.Create("PayFine_Checker", 3600, 0, function()
	    print("{MAFIARP-POLICE} Checking fines")
	    ...
	    for client, character in ix.util.GetCharacters() do
	    	...
	    	if (character:GetData("FineToPay")[1] <= os.time()) then
	    		...
	    		print("{MAFIARP-POLICE} "..character:GetName().." failed to pay fines")

	    	else

	    		local TimeString = os.date( "%H:%M:%S - %m/%d/%Y" , character:GetData("FineToPay")[1] )

	    		print("{MAFIARP-POLICE} "..character:GetName().." - Need pay before: "..TimeString)

	    	end

		end

		print("{MAFIARP-POLICE} Checking Finished")
	...
	function PLUGIN:PrepareCarVendorData()
		...
		for k, v in ipairs(PLUGIN.PoliceCarVendor) do
			local carClass = v.CarEnt
			local carIcon = v.Icon or "materials/icons/error.png"  -- Default to an error icon if not specified
			local isTaken = v.IsTaken or false  -- Default to false if not explicitly set
	
			-- Add the car's information to the table
			CarsTable[#CarsTable + 1] = {
				CarName = carClass,  -- Use the class name as the identifier
				CarIcon = carIcon,  -- Store the path to the PNG image
				CarTaken = isTaken
			}
		end
	...
	function PLUGIN:CheckClientVehicle(client)
		if not client:InVehicle() then return end
		...
		if IsValid(vehicle) then
			...
			if IsValid(parentVehicle) then 
				if not self.CarsWithLaptop[parentVehicle:GetClass()] then return end
			else
				if not self.CarsWithLaptop[vehicle:GetClass()] then return end
			end
		end
	end
...
ix.command.Add("CharSetPolice", {
	...
	OnRun = function(self, client, target)
		...
		if (target:GetData("IsPoliceOfficer", false)) then
			...
			--target:Notify("You became a police officer")
		else
			if target:GetFaction() == FACTION_POLICE then
				target:SetFaction(FACTION_CITIZEN)
			end
			client:Notify(target:GetName().." is no longer a police officer")
			--target:Notify("You are no longer a police officer")
		end

		target:SetNetVar("PoliceOfficer_Net", target:GetData("IsPoliceOfficer", false))

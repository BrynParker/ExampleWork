...
local PLUGIN = ix and ix.plugin and ix.plugin.Get and ix.plugin.Get("mafiarp_police_system") or false
-- local PLUGIN = PLUGIN
local monitorTexture = Material("mafiarp/mcpd_outline.png", "noclamp smooth" )
local computerBackground = Material("mafiarp/icons/hd/metro_dade.png", "noclamp smooth" )
local lapdlogo = Material("setorian_police/pd.png", "noclamp smooth" )

local gradient = Material("mafiarp/faction_gradient_left.png")
...
-- frame:SetSize(800,550)
	...
	self.Paint = function(s,w,h) 
		...
		surface.SetMaterial( computerBackground )
	    surface.DrawTexturedRect(170,85,w-350,h-200)
	...
	InsideMonitor:DockMargin(90,25,38,70)
...
function PANEL:LoginAnimation()
	...
	LoadingPanel:DockMargin(200,100,200,0)
	...
	CloseButton:SetPos(ScrW() * 0.05, ScrH() * 0.549)
	CloseButton:SetFont("xpgui_huge")
	CloseButton:SetContentAlignment(5)
	CloseButton:SetText( "" )
	CloseButton.Paint = function(self, w, h)
		if self:IsHovered() then
			surface.SetDrawColor(Color(79,29,29,150))
			surface.SetMaterial(Material("warfarerp/ui/ui_exit.png", "noclamp smooth"))
		else    
...
function PANEL:RenderMenu()
	...
	ButtonList:SetSize(300, 45)
	ButtonList:SetPos(ScrW() * 0.228, ScrH() * 0.509)
	ButtonList:SetZPos(99)

	local TabContentPanel = vgui.Create( "Panel", ContextPanel )
	TabContentPanel:Dock(FILL)
	TabContentPanel:DockMargin(0,0,0,0)
	...
	self.TabPanel:DockPadding(230,100,110,0)

	local buttonsFunc = {
		[1] = {
			BName = "Dashboard",
			BIcon = "mafiarp/home_icon.png",
			BMenu = function()
				self:TransitionAnim()
			end
		},
	...
	for k, v in ipairs(buttonsFunc) do
		...
		ButtonIcon:SetSize(45, 45)
		ButtonIcon:SetFont("ixIconsMedium")
		ButtonIcon:SetContentAlignment(5)
		ButtonIcon:SetText("")
		ButtonIcon.Paint = function(self, w, h)
			surface.SetDrawColor(Color(200,200,200,150))
			surface.SetMaterial(Material(v.BIcon, "noclamp smooth"))
			surface.DrawTexturedRect(0, 0, w, h)
		end
...
function PANEL:RenderDashboard()
	...
	WelcomeMsg:Dock(TOP)
	WelcomeMsg:DockMargin(-180,200,0,30)
	WelcomeMsg:SetFont("xpgui_huge")
	WelcomeMsg:SetContentAlignment(5)
	WelcomeMsg:SetText("Welcome to MDPD CAD")
	WelcomeMsg:SetAutoStretchVertical(true)
...
function PANEL:RenderPersonData()
	...
	local searchBar = vgui.Create("VoidUI.Search", self.TabPanel)
	searchBar:Dock(TOP)
	searchBar:DockMargin(0,5,0,0)
	searchBar.OnEnter = function(s)
	   	self:FillPersonData(s:GetValue())
	end

	local DataScroll = vgui.Create( "VoidUI.ScrollPanel", self.TabPanel )
	DataScroll:Dock( FILL )
	DataScroll:DockMargin(0,5,0,100)
...
function PANEL:FillPersonData(sWord)
	...
	for client, character in ix.util.GetCharacters() do
		...
		local BGPanel = vgui.Create("VoidUI.BackgroundPanel", ProfilePanel)
		BGPanel:Dock(LEFT)
		...
		mdl.PaintOver = function(s,w,h)
			...
			draw.SimpleTextOutlined("WANTED", "VoidUI.S20", w/2,h, Color( 240,math.abs(math.cos(RealTime() * 3) * 120),math.abs(math.cos(RealTime() * 3) * 120) ), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, Color(20,20,20))
		end
		...
		volitionsPanel_Button:SetText("Information")
...
function PANEL:RendersCases()
	...
	PanelBg:Dock( FILL )
	PanelBg:DockMargin(0,5,0,100)
	PanelBg:DockPadding(0,0,0,0)
	...
	local DataScroll = vgui.Create( "VoidUI.ScrollPanel", PanelBg )
	DataScroll:Dock( FILL )
	-- DataScroll.Paint = function(s,w,h)
...
function PANEL:SearchCase(sWord)
	...
	for k, v in SortedPairsByMemberValue(CasesDataBase, "CCreateTime", true) do
		...
		MoreInfoButton:SetText( (CasesInUse[k] and "Someone else is editing") or "Details")
		...
		MoreInfoButton.Think = function(s)
			...
			if (s.NextCheck < CurTime()) then
				s:SetText( (CasesInUse[k] and "Someone else is editing") or "Details")
...
function PANEL:RenderCase(CaseID,CaseData)
	...
	local sheet = vgui.Create("VoidUI.Tabs", PanelBg)
	sheet:Dock(FILL)
	sheet:DockMargin(0, 5, 0, 0)
	sheet:SetTall(15)
	sheet:SetContentAlignment(5)
	sheet:SetAccentColor(VoidUI.Colors.LightGray)
	sheet:SetMoveSpeed(0.4)
	


	----BEGIN CASES-------
	local OverviewPanel = vgui.Create("Panel", sheet)
	OverviewPanel:Dock(FILL)
	OverviewPanel:DockMargin(0, 0, 0, 0) -- Set margins to 0 to prevent offset
	...
	local overviewTab = sheet:AddTab("Overview", OverviewPanel)
	...
	local BriefTE = vgui.Create("VoidUI.TextInput", sheet)
	BriefTE:Dock(FILL)
	BriefTE:DockMargin(0, 0, 0, 0) -- Set margins to 0
	...
	sheet:AddTab("Brief", BriefTE)
	



	local CaseDetailsScroll = vgui.Create("DScrollPanel", sheet)
	CaseDetailsScroll:Dock(FILL) -- Dock fill to ensure it fills the parent
	CaseDetailsScroll:DockMargin(0, 0, 0, 0) -- Set margins to 0
	...
	for k, v in SortedPairs(self.PersonCaseData) do
		...
		PersonsPanel:DockMargin(0, 5, 0, 5) -- Adjusted to consistent margins
		PersonsPanel:DockPadding(0, 5, 0, 5) -- Adjusted to consistent padding
		...
		PersonsPanel_Title:SetFont("xpgui_small")
	...
	sheet:AddTab("Case Details", CaseDetailsScroll)

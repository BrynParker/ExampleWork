...
function PANEL:Init()
	...
	self.WantedButton:SetText( "Place Warrant" )
	...
	WantedStatus:SetText( "Warrant Status" )
	...
	WantedStatusPnl:DockMargin(5,110,5,0)
...
function PANEL:PopulateScroll(p, RPTCriminalRecord)
	...
	if (p:isWanted()) then
		...
		self.WantedButton:SetText( "Remove Warrant" )
	end
	...
	self.WantedButton.DoClick = function(s)
		...
		if (self.wReason == "") and (!p:isWanted()) then
			LocalPlayer():Notify("Give a reason for the warrant")
			return
		end
...
vgui.Register("ixPoliceSys_PersonProfile", PANEL, "VoidUI.Frame")

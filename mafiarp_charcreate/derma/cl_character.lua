...
local cameraPos = Vector(4637.666504, -3996.064453, 183.300430)
local cameraAng = Angle(3.644962, 90.319580, 0.000000)
...
function PANEL:createTitle()
	...
	self.title:SetText("Civic Networks")
	...
	self.desc:SetText("Mafia Roleplay")
...
local bg = ix.util.GetMaterial("materials/warfarerp/ui/stack_circle.png")
...
function PANEL:fadeOut()
    self:AlphaTo(0, self.ANIM_SPEED, 0, function()
        ...
        -- Restore player's view to their current position and angles
        hook.Remove("CalcView", "MainMenuCameraView")
        if IsValid(LocalPlayer()) then
            LocalPlayer():SetEyeAngles(LocalPlayer():EyeAngles())  -- Set to current angles
        end
    end)
end
...
function PANEL:Init()
	if IsValid(LocalPlayer()) then
        originalPos = LocalPlayer():GetPos()  -- Capture original position when panel is initialized
        originalAng = LocalPlayer():EyeAngles()  -- Capture original angle when panel is initialized
    end
    ...
    hook.Add("CalcView", "MainMenuCameraView", function(player, pos, angles, fov)
        if not menuIsOpen then return end  -- Ensure the camera override only applies when menu is open
        local view = {}

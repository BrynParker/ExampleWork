...
-- frame:SetSize(800,550)
	...
	self.Paint = function(s,w,h)
        ...
        surface.SetDrawColor(ColorAlpha(Color(255, 255, 255), 255))
		...
		surface.SetMaterial(Material("warfarerp/ui/stack_circle.png"))
	    surface.DrawTexturedRect(0,0,w,h)

	    --draw.SimpleText("Metro-Dade County Bank", "ix3D2DMediumFont", 12,5, Color( 240,240,240 ), TEXT_ALIGN_LEFT,TEXT_ALIGN_LEFT)
	...
	Character:DockMargin(0,0,ScreenScale(10),0)
	...
	Character:Dock(LEFT)
	Character:DockMargin(ScreenScale(15),0,0,0)
	Character:SetWide(ScreenScale(160))
	Character:SetModel("models/player/suits/group2/male_08_open.mdl")
	Character:SetFOV(modelFOV)
	...
	GreetingMsg.Paint = function(s,w,h)
		...
		surface.SetMaterial(Material("mafiarp/button_rounded.png"))
	    surface.DrawTexturedRect(0,0,w,h)

		draw.SimpleText("Welcome to the Metro-Dade County Bank", "ixMediumFont", w/2, h/2, Color( 255,255,255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
function PANEL:RenderMenu(bMenu, noAnim)
	...
	if (bMenu == 6) then
		...
		if (table.IsEmpty(accountsWithLoan)) then
			VoidLib.Notify("", "YOU DO NOT HAVE ANY LOANS TO PAY", Color(58, 108, 69), 4)
			return
		end
	...
	for k, v in SortedPairs(MenuButtons) do
		...
		Button1.Paint = function(s,w,h)
			if Button1:IsHovered() then 
				surface.SetDrawColor(255,255,255,255)
				surface.SetMaterial(Material("mafiarp/button_black.png"))
				surface.DrawTexturedRect(0,0,w,h)
			else
				surface.SetDrawColor(255,255,255,255)
				surface.SetMaterial(Material("mafiarp/button_rounded.png"))
			...
			draw.SimpleText(v.Display, "ixMediumFont", 20, h/2, Color( 255,255,255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
		Button1.DoClick = function()
			if (v.CustomCheck()) then
				self:RenderMenu(v.Menu)
			else
				VoidLib.Notify("", v.CustomCheckMsg or "You can't do that", Color(170, 66, 66), 4)
			end
		end
...
function PANEL:RenderCreateAccount()
	...
	BackButton.Paint = function(s,w,h)
		...
		draw.SimpleText("Back", "ixMediumFont", 20, h/2, Color( 255,255,255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	...
	InfoPanelText:SetTextColor(Color(255,255,255))
	...
	PinEntryText:SetTextColor(Color(255,255,255))
	...
	CreateButton.Paint = function(s,w,h)
		...
		draw.SimpleText("Create", "ixMediumFont", 20, h/2, Color( 255,255,255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	CreateButton.DoClick = function()

		if (PinEntry:GetValue() == "") then
			VoidLib.Notify("", "PIN CANNOT BE EMPTY", Color(170, 66, 66), 4)
			return
		end

		if (string.len(PinEntry:GetValue()) != 4) then
			VoidLib.Notify("", "PIN IS TOO SHORT", Color(170, 66, 66), 4)
			return
		end
...
function PANEL:RenderDeleteAccount()
	...
	for k, v in pairs(self.AccountsData or {}) do
		...
		AccountBG.DoClick = function()
			...
			CustomDermaQuery( "Are you sure?", "Deletion warning", "Yes", function()
...
function PANEL:RenderLostCard()
	...
	for k, v in pairs(self.AccountsData or {}) do
		...
		AccountBG.DoClick = function(s, w, h)
			surface.PlaySound("helix/ui/press.wav")
	...
	ChangePINButton.Paint = function(s,w,h)
		if ChangePINButton:IsHovered() then 
			surface.SetDrawColor(255,255,255,255)
			surface.SetMaterial(Material("mafiarp/button_black.png"))
			surface.DrawTexturedRect(0,0,w,h)
		else
		...
		draw.SimpleText("Replace Card", "ixMediumFont", 20, h/2, Color( 255,255,255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	ChangePINButton.DoClick = function()

		if (self.SelectedAccount == "") then
			VoidLib.Notify("", "PLEASE SELECT AN ACCOUNT", Color(170, 66, 66), 4)
			return
		end
...
function PANEL:RenderChangePin()
	...
	local DScrollBG = vgui.Create( "DPanel", self.mainPanel )
	DScrollBG:Dock(TOP)
	DScrollBG:SetTall(ScreenScale(70))
	DScrollBG:DockMargin(ScreenScale(35),10,ScreenScale(35),10)
	DScrollBG.Paint = function(s,w,h)
		surface.SetDrawColor(255,255,255,255)
		surface.SetMaterial(Material("mafiarp/button_rounded.png"))
	...
	ChangePINButton.Paint = function(s,w,h)
		...
		draw.SimpleText("Change PIN", "ixMediumFont", 20, h/2, Color( 255,255,255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	ChangePINButton.DoClick = function()

		if (self.SelectedCardID == "") then
			VoidLib.Notify("", "PLEASE SELECT A CARD", Color(170, 66, 66), 4)
			return
		end
...
function PANEL:RenderCreateGroupAccount()
	...
	CreateButton.DoClick = function()
		...
		if (NameEntry:GetValue() == "") then
			VoidLib.Notify("", "NAME CANNOT BE EMPTY", Color(170, 66, 66), 4)
			return
		end

		if (string.len(NameEntry:GetValue()) < 4) then
			VoidLib.Notify("", "NAME IS TOO SHORT", Color(170, 66, 66), 4)
			return
		end

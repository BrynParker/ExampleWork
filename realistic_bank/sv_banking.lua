...
function PLUGIN:AssignSafetyBox(bankID, boxType, char)
    ...
    -- else
        ...
        ix.item.NewInv(char:GetID() or 0, boxInvType, function(inv)
...
function PLUGIN:GroupAccountsCount(ownerID, ply)
    ...
    self:GetAllAccountsData(function(data)
        for k, v in pairs(data) do
            if tonumber(v.owner) == ownerID and v.is_groupaccount and tobool(v.is_groupaccount) then
                acc_count = acc_count + 1
            end
        end
    end)

    local extraGroupAccounts = PLUGIN.ExtraGroupAccounts[ply:GetUserGroup()] or 0

    if acc_count >= (ix.config.Get("MaxGroupAccountPerChar", 5) + extraGroupAccounts) then
        return false
    else
        return true
    end
